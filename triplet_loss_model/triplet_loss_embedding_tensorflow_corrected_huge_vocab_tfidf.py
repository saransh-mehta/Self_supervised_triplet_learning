...
DATA_DICTI_PATH = 'demand_supply_resume_relavancy_data_project_skills_tfidf.dicti'
...
SEED = 2
SAVE_DIR = os.path.join(os.getcwd(), 'saved_model_huge_vocab_tfidf')
LOG_DIR = SAVE_DIR
EMBED_SAVE = os.path.join(SAVE_DIR, 'improved_embeddings_with_triplet_loss_huge_vocab_tfidf.npy')
...
def get_triplet_batch(dataDicti, batchSize):
	...
	# as we are using the data already sorted in descending according to tf-idf score
	# we will select negatives only from top 10 tf-idf from 1_2 of tht demand

	negativesList = []

	#print('10 negatives: ',dataDicti[randomDemand][3][:10])
	#print('intersection: ', dataDicti[randomDemand][2])

	# using tfidf for negatives, generates a prolem for negative, 
	# as top 10 negative mostly occur in positives, hence we keep the 
	# order descending and go through each of them in descending order
	# keep it in list in it is not in positive.
	# hence, not use [:10] here, this doesn't happen with positives
	# as there is no if condition on it.
	...
	#print('negative list: ',negativesList)
	...
	# as we are using the data already sorted in descending according to tf-idf score
	# we will select positives only from top 10 tf-idf from 3_4 of tht demand

	positivesList = []
	for a in dataDicti[randomDemand][2][:10]:
		if a != anchor:
			string = a.strip()
			string = preprocessor.replace_plus(string)
			string = preprocessor.replace_plus1(string)
	...
	#print('positive list: ',positivesList)
